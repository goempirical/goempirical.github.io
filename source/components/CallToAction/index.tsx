...
function CallToAction({ className, link, title }: ICallToActionProps) {
  return (
    <div className={clsx('flex flex-col gap-6 items-center', className)}>
      ...
      <a href={link.href} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center bg-primary rounded-full w-auto px-8 py-2 font-bold text-center text-white" rel="noreferrer">{link.title}</a>
    </div>
  );
}
